...
def save_ntk_stds(path):
    ...
    num_examples = 0
    for i, (X, y) in enumerate(tqdm(it)):
    ...
    test_std = torch.sqrt((sums_sqr / num_examples) - (sums / num_examples) ** 2).data.cpu().numpy()
    test_std = np.nan_to_num(test_std, 0.0)
    save_path = (path +
                 '/stds_num_classes_' + str(num_classes) +
                 '_noise_' + str(label_noise) +
                 '_lambda_log_' + str(lambda_log) +
                 '_seed_' + str(seed) +
                 '_epoch_' + str(id_epoch))

    np.save(save_path, np.concatenate([flat_params.detach().cpu().numpy()[None], test_std[None]], 0))

    # train_std = torch.sqrt((sums_sqr / sums.shape[0]) - (sums / sums.shape[0]) ** 2)
    # flat_params = flat_params.data.cpu().numpy()
    ...
    #     sums += jac.sum(0)
    #     sums_sqr = (jac ** 2).sum(0)
    # np.save(save_path, np.concatenate([flat_params, test_std.data.cpu().numpy(), train_std.data.cpu().numpy()], 0))
...
# train the model
save_path = '/home/pezeshki/scratch/dd/DD/' + exp_name
...
for id_epoch in range(train_epoch):
    ...
    if compute_ntk and id_epoch * 10 == 0:
        save_ntk_stds(save_path)
